...
private fun ChatTab(
    ...
    Column(verticalArrangement = Arrangement.spacedBy(8.dp), modifier = Modifier.fillMaxSize()) {
        Row(horizontalArrangement = Arrangement.spacedBy(8.dp), modifier = Modifier.fillMaxWidth()) {
            OutlinedTextField(value = state.currentUserId, onValueChange = onUserIdChanged, label = { Text("User ID") }, modifier = Modifier.weight(1f), singleLine = true)
            OutlinedTextField(value = state.roomId, onValueChange = onRoomChanged, label = { Text("Room") }, modifier = Modifier.weight(1f), singleLine = true)
        }

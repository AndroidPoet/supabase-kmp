...
hand it the same bucket and path and get the bytes back.
...
<Callout type="info">
  **Pause / resume across restarts.** Cancelling the coroutine running `await()`
  ...
  current offset and continues. Chunks default to 6 MiB (a *mebibyte* is
  1,024 KiB ≈ 6.3 MB) (`RESUMABLE_DEFAULT_CHUNK_SIZE`), the size Supabase
  requires.
</Callout>
...
to `getPublicUrl`, `createSignedUrl`, or `download` and Supabase resizes and
re-encodes the file on the fly — so the same original can serve a 200×200
thumbnail and a full-size view from one stored file:

...
Subscribes to `channelName` and registers a single `postgres_changes` callback in one
...
Subscribes to `channelName` and registers a single broadcast `callback` for the named
`event` in one call.
...
Subscribes to `channelName` and registers a single presence sync `callback`, invoked

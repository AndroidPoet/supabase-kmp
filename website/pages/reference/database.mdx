...
`range` — **not** a `Pair`). The typed [`selectWithCountTyped`](#selectwithcount) sits
...
The GET counterpart of `rpcSingleTyped`. Fails with HTTP 406 / `NOT_FOUND` when not
...
The GET, lenient counterpart of `rpcSingleTyped`: "no rows" (HTTP 406 / `NOT_FOUND`)
...
Produced by [`selectWithCountTyped`](#selectwithcount).

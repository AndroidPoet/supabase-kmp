...
[`createSignedUploadUrl`](#createuploadsignedurl)).
...
[`ObjectListV2Response`](#objectlistv2result) that separates folders from objects
...
```kotlin
val token = storage.createSignedUploadUrl("avatars", "jane/profile.png")
```

### createSignedUploadUrlWithPath

Like [`createSignedUploadUrl`](#createuploadsignedurl) but returns the full
...
```kotlin
val signed = storage.createSignedUploadUrlWithPath("avatars", "jane/profile.png")
```
...
[`createSignedUploadUrlWithPath`](#createuploadsignedurlwithpath).
...
```kotlin
enum class VectorDistanceMetric { COSINE, EUCLIDEAN, DOT_PRODUCT, UNKNOWN }
```

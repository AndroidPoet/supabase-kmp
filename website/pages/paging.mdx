...
- `isEndReached` — `true` once there are no more rows
...
fun TodosScreen(vm: TodosViewModel) {
    ...
    val isEndReached by vm.pager.isEndReached.collectAsState()
    val error by vm.pager.error.collectAsState()

    LazyColumn {
        itemsIndexed(todos, key = { _, t -> t.id }) { index, todo ->
            TodoRow(todo)
            if (index >= todos.lastIndex - 3 && !isEndReached) {
...
while (!pager.isEndReached.value) {
    pager.loadNext()
...
| You get | `items`/`isLoading`/`isEndReached`/`error` + `loadNext` | + auto-prefetch, placeholders, `LazyColumn` binding |

...
description: Invoke Supabase Edge Functions by name from Kotlin with typed request and response bodies, region pinning, and forwarded auth for RLS.
---
...
The plain `invoke` hands you the response as a raw `String`, which is useful when the
function returns something that isn't JSON, or when you'd rather parse it yourself.
For *fire-and-forget* calls — where you only need to know the call succeeded, as when
triggering a background job — call `invoke` and ignore the value (or `.map {}` it to
`Unit`); the `SupabaseResult` still surfaces a non-2xx response as a failure.
